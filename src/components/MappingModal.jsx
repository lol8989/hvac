...
import { ratioOf, ROOMS, ODU_CATALOG } from '../data.js'

function Chip({ id, from, onUnassign }) {
  const r = ROOMS[id]
  return (
    <div
      className="chip"
...
export default function MappingModal({ groups, pool, onMove, onReplace, onSplit, onAddGroup, onRemove, onClose, onApply }) {
  ...
  const catOf = (model) => ODU_CATALOG.find((c) => c.model === model)
    ...
    >
      <div className="modal">
        ...
        <div className="m-toolbar">
          ...
          <button className="btn sm" onClick={() => onAddGroup(catOf(addModel))}>+ 추가</button>
        </div>
