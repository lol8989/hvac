...
describe('Placement.ai (생성/검증)', () => {
  it('AI 최초 선정으로 생성하면 effectiveSelection이 AI값이고 오버라이드가 아니다', () => {
    const p = Placement.ai('room-1', sel('RNW0401C2S', 2), pos(2))
    expect(p.roomId).toBe('room-1')
    expect(p.effectiveSelection).toEqual({ modelCode: 'RNW0401C2S', quantity: 2 })
  ...
  it('roomId가 빈값이면 생성 시 에러를 던진다', () => {
    expect(() => Placement.ai('', sel('RNW0401C2S', 1), pos(1))).toThrow()
    expect(() => Placement.ai('   ', sel('RNW0401C2S', 1), pos(1))).toThrow()
  })
  ...
  it('quantity가 0/-1/1.5(1 미만 또는 비정수)이면 에러를 던진다', () => {
    expect(() => Placement.ai('room-1', sel('RNW0401C2S', 0), [])).toThrow()
    expect(() => Placement.ai('room-1', sel('RNW0401C2S', -1), [])).toThrow()
    expect(() => Placement.ai('room-1', sel('RNW0401C2S', 1.5), pos(2))).toThrow()
  })
})

describe('Placement 불변식: positions.length === quantity', () => {
  it('[적대] 좌표 개수가 대수와 다르면 생성할 수 없다', () => {
    expect(() => Placement.ai('room-1', sel('RNW0401C2S', 2), pos(1))).toThrow(/좌표 개수/)
    expect(() => Placement.ai('room-1', sel('RNW0401C2S', 2), pos(3))).toThrow(/좌표 개수/)
  })

  it('[적대] 좌표가 유한수가 아니면 에러', () => {
    expect(() => Placement.ai('room-1', sel('RNW0401C2S', 1), [{ x: NaN, y: 0, rot: 0 }])).toThrow()
    expect(() => Placement.ai('room-1', sel('RNW0401C2S', 1), [{ x: 0, y: Infinity, rot: 0 }])).toThrow()
  })

  it('[적대] 오버라이드 시에도 좌표 개수가 맞아야 한다', () => {
    const p = Placement.ai('room-1', sel('RNW0401C2S', 2), pos(2))
    expect(() => p.overrideSelection(sel('RNW0601C2S', 3), pos(2))).toThrow(/좌표 개수/)
  })
})

describe('Placement 오버라이드 (사용자 조정)', () => {
  it('overrideSelection하면 effectiveSelection이 사용자값으로 바뀌고 isOverridden이 true다', () => {
    const p = Placement.ai('room-1', sel('RNW0401C2S', 2), pos(2)).overrideSelection(sel('RNW0601C2S', 3), pos(3))
    expect(p.effectiveSelection).toEqual({ modelCode: 'RNW0601C2S', quantity: 3 })
  ...
  it('overrideSelection에 잘못된 선정값(빈 modelCode, quantity 0)을 주면 에러를 던진다', () => {
    ...
    expect(() => p.overrideSelection(sel('RNW0601C2S', 0), [])).toThrow()
  })

  it('withAiSelection(AI 재선정)해도 사용자 오버라이드가 보존된다', () => {
    const p = Placement.ai('room-1', sel('RNW0401C2S', 2), pos(2))
      .overrideSelection(sel('RNW0601C2S', 3), pos(3))
  ...
  it('오버라이드된 실은 AI 재선정이 좌표도 건드리지 않는다(사용자가 놓은 자리 유지)', () => {
    ...
    const p = Placement.ai('room-1', sel('RNW0401C2S', 2), pos(2))
      .overrideSelection(sel('RNW0601C2S', 1), mine)
  ...
  it('오버라이드가 없으면 withAiSelection이 effectiveSelection과 좌표를 새 AI값으로 바꾼다', () => {
    const p = Placement.ai('room-1', sel('RNW0401C2S', 2), pos(2)).withAiSelection(sel('80C', 1), pos(1))
...
describe('Placement 도면 편집 — 좌표만 (대수 불변)', () => {
  it('moveUnit은 그 심볼의 좌표만 바꾸고 오버라이드를 만들지 않는다', () => {
    const p = Placement.ai('room-1', sel('RNW0401C2S', 2), pos(2)).moveUnit(1, 55, 66)
  ...
  it('rotateUnit은 회전만 바꾼다', () => {
    const p = Placement.ai('room-1', sel('RNW0401C2S', 1), pos(1)).rotateUnit(0, 90)
...
describe('Placement 도면 편집 — 대수가 바뀐다 (사용자 오버라이드)', () => {
  it('addUnit은 대수를 1 늘리고 오버라이드로 기록한다 (모델은 유지)', () => {
    const p = Placement.ai('room-1', sel('RNW0401C2S', 1), pos(1)).addUnit({ x: 9, y: 9, rot: 0 })
    expect(p.quantity).toBe(2)
    expect(p.effectiveSelection).toEqual({ modelCode: 'RNW0401C2S', quantity: 2 })
    expect(p.isOverridden).toBe(true)
  })
  ...
  it('removeUnit은 대수를 1 줄이고 오버라이드로 기록한다', () => {
    const p = Placement.ai('room-1', sel('RNW0401C2S', 3), pos(3)).removeUnit(1)!
    expect(p.quantity).toBe(2)
  ...
  it('마지막 한 대를 지우면 null을 반환한다(그 실에 실내기가 없다)', () => {
    expect(Placement.ai('room-1', sel('RNW0401C2S', 1), pos(1)).removeUnit(0)).toBe(null)
  })

  it('추가·삭제 후에도 AI 재선정은 사용자 대수를 덮지 않는다', () => {
    const p = Placement.ai('room-1', sel('RNW0401C2S', 1), pos(1))
      .addUnit({ x: 9, y: 9, rot: 0 })
      .withAiSelection(sel('RNW0401C2S', 1), pos(1))
    expect(p.quantity).toBe(2)
  })
...
describe('Placement.totals (용량 합산)', () => {
  it('40C(4000/4500) 3대이면 coolW 12000 / heatW 13500이다 (엑셀 실측)', () => {
    const p = Placement.ai('room-1', sel('RNW0401C2S', 3), pos(3))
    expect(p.totals(model40C)).toEqual({ coolW: 12000, heatW: 13500 })
  })

  it('model.model가 effectiveSelection.modelCode와 다르면 에러를 던진다 (정합 보호)', () => {
    const p = Placement.ai('room-1', sel('RNW0601C2S', 2), pos(2))
    expect(() => p.totals(model40C)).toThrow()
  })

  it('오버라이드된 selection 기준으로 totals를 계산한다', () => {
    const p = Placement.ai('room-1', sel('RNW0601C2S', 2), pos(2)).overrideSelection(sel('RNW0401C2S', 2), pos(2))
    expect(p.totals(model40C)).toEqual({ coolW: 8000, heatW: 9000 })
  })

  it('도면에서 1대를 지우면 총용량도 줄어든다(대수 SSOT)', () => {
    const p = Placement.ai('room-1', sel('RNW0401C2S', 3), pos(3)).removeUnit(0)!
    expect(p.totals(model40C)).toEqual({ coolW: 8000, heatW: 9000 })
  })
})
...
describe('Placement 불변성/동등성', () => {
  it('인스턴스와 selection은 동결(frozen)되어 있고, 갱신 메서드는 새 인스턴스를 반환한다', () => {
    ...
    const q = p.overrideSelection(sel('RNW0601C2S', 3), pos(3))
    expect(q).not.toBe(p)
    expect(p.effectiveSelection).toEqual({ modelCode: 'RNW0401C2S', quantity: 2 }) // 원본 비파괴
  })

  it('equals는 roomId가 같으면 true, 다르면 false다', () => {
    const a = Placement.ai('room-1', sel('RNW0401C2S', 2), pos(2))
    const b = Placement.ai('room-1', sel('RNW0601C2S', 5), pos(5))
    const c = Placement.ai('room-2', sel('RNW0401C2S', 2), pos(2))
