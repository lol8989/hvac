...
describe('applyAiPlacement', () => {
  it('placement가 없는 실이면 Placement.ai로 새로 생성한다', () => {
    ...
    expect(result['r-a'].effectiveSelection).toEqual({ modelCode: 'RNW0401C2S', quantity: 1 })
  ...
  it('오버라이드 없는 기존 placement는 AI 추천으로 갱신된다', () => {
    const placements = { 'r-a': ai('r-a', 'RNW0701C2S', 9) }
  ...
  it('오버라이드 있는 실은 effectiveSelection을 유지하고 ai값만 갱신한다 (AI 재실행 시 수정 셀 보존)', () => {
    const overridden = ai('r-a', 'RNW0401C2S', 9).overrideSelection({ modelCode: 'RNW0701C2S', quantity: 5 }, pos(5))

    const result = applyAiPlacement([roomA], { 'r-a': overridden }, models, layoutFor)

    expect(result['r-a'].effectiveSelection).toEqual({ modelCode: 'RNW0701C2S', quantity: 5 }) // 수정 셀 보존
    expect(result['r-a'].isOverridden).toBe(true)
    expect(result['r-a'].selection.ai).toEqual({ modelCode: 'RNW0401C2S', quantity: 1 }) // ai는 최신 추천
  ...
  it('실 면적을 변경한 뒤 재실행하면 추천이 바뀐다 (상류 수정 → 하류 재계산 연쇄)', () => {
    ...
    expect(before['r-a'].effectiveSelection).toEqual({ modelCode: 'RNW0401C2S', quantity: 1 })
    ...
    expect(after['r-a'].effectiveSelection).toEqual({ modelCode: 'RNW0701C2S', quantity: 1 })
  })

  it('원본 placements Record와 기존 Placement를 파괴하지 않는다', () => {
    const original = ai('r-a', 'RNW0701C2S', 9)
    ...
    expect(original.effectiveSelection).toEqual({ modelCode: 'RNW0701C2S', quantity: 9 })
  })

  it('rooms 목록에 없는 실의 기존 placement는 그대로 유지된다', () => {
    const other = ai('r-other', 'RNW0701C2S', 2)
...
describe('placementTotalsW', () => {
  it('40C 3대이면 coolW 12000 / heatW 13500이다 (엑셀 실측)', () => {
    expect(placementTotalsW(ai('r-a', 'RNW0401C2S', 3), models)).toEqual({ coolW: 12000, heatW: 13500 })
  })

  it('오버라이드가 있으면 유효 선정(user) 기준으로 계산한다', () => {
    const p = ai('r-a', 'RNW0401C2S', 3).overrideSelection({ modelCode: 'RNW0701C2S', quantity: 2 }, pos(2))
...
describe('groupIndoorTotalsW', () => {
  it('그룹 내 실들의 유효 배치 총용량을 합산한다', () => {
    const placements = {
      'r-a': ai('r-a', 'RNW0401C2S', 3), // 12000/13500
      'r-b': ai('r-b', 'RNW0701C2S', 1), // 7000/8000
    }
  ...
  it('placement가 없는 실은 0으로 계상한다', () => {
    const placements = { 'r-a': ai('r-a', 'RNW0401C2S', 3) }
  ...
  it('그룹에 속하지 않은 실의 placement는 합산하지 않는다', () => {
    const placements = {
      'r-a': ai('r-a', 'RNW0401C2S', 3),
      'r-out': ai('r-out', 'RNW0701C2S', 9),
    }
