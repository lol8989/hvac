// 제품(모델) 리포지토리 — 읽기(4단 분류 조인 평탄 조회) + 쓰기(등록·수정·게시 전이·업로드).
// 도메인 불변식(게시본 잠금·허용 전이·필드 유효성)을 먼저 강제한 뒤 트랜잭션으로 반영한다.

import type { BulkStatusResult, ProductRow, SeriesOption } from '../../../application/equipment/adminPorts'
...
export class SqliteProductRepository {
  ...
  setStatus(id: number, next: PublishStatus): void {
    ...
    const ts = this.tx.now()
    this.tx.run(() => this.applyStatus(id, next, ts))
  }
  ...
  setStatusMany(ids: readonly number[], next: PublishStatus): BulkStatusResult {
    ...
    const ts = this.tx.now()
    this.tx.run(() => {
      for (const id of targets) this.applyStatus(id, next, ts)
    })
  ...
  // 게시 상태 전이 UPDATE(단건·일괄 공용). 게시 시각은 게시할 때만 기록하고,
  // 보관 해제(재게시) 시 단종일을 해제한다.
  private applyStatus(id: number, next: PublishStatus, ts: string): void {
